...
class TableNonCableCart extends StatefulWidget {
  const TableNonCableCart({super.key, required this.idLoading});
...
class _TableNonCableCartState extends State<TableNonCableCart> {
  ...
  void getDataLoading() async {
    ...
    try {
      ...
      setState(() {
        LoadingByIdSparekit = response!.data['loading'][0]['cables_id'];
      });
    } catch (e) {
      QuickAlert.show(
          context: context,
  ...
  //               onConfirmBtnTap: () {
  //                 hapusDataSparekitLoading('${data['_id']['id']}');
  //               });
  //         },
  //         child: Text("Delete",
  //             style: GoogleFonts.montserrat(
